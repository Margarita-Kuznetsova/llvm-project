//===- loveMatInt.cpp - Immediate materialisation -------------*- C++ -*--===//
//
...
static int getInstSeqCost(loveMatInt::InstSeq &Res, bool HasRVC) {
  if (!HasRVC)
    return Res.size();
...
static void generateInstSeqImpl(int64_t Val,
  ...
  if (isInt<32>(Val)) {
    ...
    if (Hi20)
      Res.push_back(loveMatInt::Inst(love::LUI, Hi20));

    if (Lo12 || Hi20 == 0) {
      unsigned AddiOpc = love::ADDI;
      Res.push_back(loveMatInt::Inst(AddiOpc, Lo12));
    }
    return;
  }
