...
#include "loveGenAsmWriter.inc"

void loveInstPrinter::printRegName(raw_ostream &O, unsigned RegNo) const {
  O << getRegisterName(RegNo);
}

void loveInstPrinter::printInst(const MCInst *MI, uint64_t Address,
...
void loveInstPrinter::printOperand(const MCInst *MI, int OpNo, raw_ostream &O) {
...
void loveInstPrinter::printBranchOperand(const MCInst *MI, uint64_t Address,
...
const char *loveInstPrinter::getRegisterName(unsigned RegNo) {
  return getRegisterName(RegNo,
                         ArchRegNames ? love::NoRegAltName : love::ABIRegAltName);
}
